...
-- часов не пережидали даже одну смену: повесил утром — к вечеру уже нет, а
-- ролик и пост в канале ещё висят и ведут в пустоту. Сутки — это ровно тот
-- срок, за который объявление увидят и те, кто заходит на сайт вечером.
...
-- доске нужна ровно на день. Срок хранится в expires_at, а не считается
...
-- сутки, отпугивает больше, чем спасает. У гостя вместо user_id — guest_id,
...
-- объявление на день, а не доступ к аккаунту.
