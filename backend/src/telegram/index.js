...
async function start() {
  ...
  // До настройки вебхука: объявления, отложенные прошлым процессом, должны
  // встать в очередь раньше того, что придёт сейчас, — они и ждут дольше.
  // Своих ошибок эта функция наружу не отдаёт, но упасть на ней бот не вправе:
  // не поднялось отложенное — работаем дальше без него.
  await bot.restoreDeferred().catch((err) => console.error('Отложенные объявления:', err));
  // Слежка за просмотрами рекламы и за заявками в ответах Threads. Обе
  // отчитываются в Telegram, поэтому и живут вместе с ботом.
  bot.startWatchers();
