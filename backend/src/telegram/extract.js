...
const GROQ = {
  ...
  // Отдельная модель для пересланного текста. Минутный лимит она не поднимает —
  // у всех обычных моделей Groq на бесплатном тарифе он одинаковый, 8000, и
  // считается на организацию. Смысл в другом: тексту не нужно зрение, а значит,
  // для него открыты модели, которых нет среди зрячих, — и первый кандидат
  // groq/compound с его 70000 в минуту (правда, всего 250 запросов в сутки и с
  // собственным веб-поиском внутри). Не задана — текст разбирает та же модель,
  // что и картинки.
...
// image = false для пересланного текста: правила про ленту чата, нечитаемый
// текст и интерфейс мессенджера ему не нужны, а лимит у Groq считается на весь
// запрос сразу — каждая лишняя тысяча знаков промпта отнимает место у самого
// объявления. На тексте из-за этого отбивались объявления, в которых не было
// ничего большого (см. TEXT_STEPS ниже).
function buildSystem(categories, { image = true } = {}) {
  const onlyImage = (line) => (image ? [line] : []);
  return [
    ...
    '',
    ...onlyImage('Скриншот из WhatsApp или Telegram — это лента чата, и почти всегда на нём несколько разных объявлений от разных людей. Пройди сообщения сверху вниз и разбери КАЖДОЕ отдельным элементом массива. Сколько объявлений видно — столько элементов и верни.'),
    ...onlyImage('Не останавливайся на первом объявлении. Не объединяй два объявления в одно, даже если они похожи по смыслу или идут подряд: разные сообщения — разные элементы. Обрезанное сверху или снизу сообщение бери, если по видимому тексту понятно, что нужно и кому.'),
    '',
    ...
    '- is_listing = false для вербовки под видом работы для девушек: так в кыргызстанских чатах пишут сутенёры, а по форме это «уборка» или «помощь по дому». Обязательный признак — НЕ НАЗВАНО, ГДЕ И ЧТО ДЕЛАТЬ: у настоящей уборки есть объект (квартира после ремонта, дом, офис, кафе, подъезд) и обычно район. Объекта нет — смотри остальное: зовут только девушек («кыздар», «келиндер», «жаштар кыздар», «девушки»); деньги сразу и наличными («акчасы налчи сразу берем», «оплата сразу», «жакшы толонот»); вместо места работы встреча или ориентир («Ош базар жактан», «подъезжай»); зовут писать в личку («жазгыла лчка»); названы возраст и внешность; за простую работу платят заметно больше обычного.',
    '  Нет объекта плюс хоть один такой признак — is_listing = false, listing_type = "other", в note «похоже на вербовку». Лишний отказ тут не страшен: уборщица пришлёт объявление заново, а опубликованная вербовка — это живые люди, которых мы к ней привели. Обычную уборку правило не трогает: «нужна уборщица в кафе на Чуй», «убрать квартиру после ремонта, 2-мкр» — объект назван.',
    ...
    ...onlyImage('- is_listing = false, если текст на скриншоте не читается, обрезан так, что суть непонятна, или ты не уверен, что разобрал его правильно. Не додумывай содержание по одному-двум словам и не восстанавливай смысл по догадке.'),
    ...onlyImage('- Не принимай за объявление элементы интерфейса мессенджера: имя чата и контакта, дату, «в сети», подписи вложений, названия групп, текст закреплённого сообщения.'),
    ...
    '',
    'Перед ответом перечитай каждый элемент с is_listing = true и проверь: понятно, что именно предлагают или ищут? если это заказ или вакансия — названа ли профессия, а не только график и возраст? если зовут девушек на уборку или помощь по дому — назван ли объект, или это вербовка? нет ли здесь запрещённого? текст ты действительно прочитал, а не додумал? Если хоть на один вопрос ответ «нет» — поставь is_listing = false. И отдельно проверь тип: заказчику нужен человек → "order" или "vacancy"; человек предлагает себя, товар или жильё → "board".',
  ].join('\n');
}
...
async function call(lane, system, content, { maxTokens = DEFAULT_MAX_TOKENS, image = true } = {}) {
  const provider = lane.provider;

  const body = JSON.stringify({
    // Пересланный текст можно разбирать моделью без зрения — они у Groq крупнее
    // и с более щедрым минутным лимитом. Задаётся GROQ_TEXT_MODEL; не задана —
    // работаем одной моделью на всё, как раньше.
    model: (!image && provider.textModel) || provider.model,
    temperature: 0,
...
async function ask(content, systemSuffix, { maxTokens = DEFAULT_MAX_TOKENS, image = true } = {}) {
  const lane = pickLane();
  if (!lane) throw new Error('Не задан GROQ_API_KEY (или FALLBACK_API_URL)');

  const categories = await categoriesRepo.listNames();
  const base = buildSystem(categories, { image });
  ...
  for (const next of lanesFrom(lane)) {
    ...
    try {
      data = await call(next, system, content, { maxTokens, image });
      lastErr = null;
      break;
    } catch (err) {
      lastErr = err;
...
// Ступени на случай «Request too large». Считать тут надо не байты, а токены
// минутного лимита, и раскладка такая: системный промпт ≈3800, картинка у Qwen
// стоит ровно 2048 независимо от своего размера, плюс потолок ответа, который
// Groq резервирует заранее. При потолке 2600 сумма выходила 8431 при лимите
// 8000 — то есть скриншот не влезал никогда, а у рекламы к промпту добавлялась
// ещё и приписка AD_SUFFIX.
//
// Поэтому главный рычаг — потолок ответа, а не пиксели: 1500 хватает примерно
// на пять разобранных объявлений, больше на один скриншот и не попадает.
// Было 1800, но правило про вербовку добавило промпту около пятисот токенов, и
// сумма вылезла за 8000 — потолок ответа пришлось подвинуть на ту же величину.
// Если промпт снова вырастет, двигать надо это число: считать сумму
// (промпт + 2048 + потолок) обязательно, иначе скриншоты перестанут влезать все
// разом, а выглядеть это будет как «Groq сломался».
// Пиксели ужимаем следом, во вторую очередь: на цену картинки у Qwen они не
// влияют, но у запасного шлюза (FALLBACK_API_URL) счёт может быть и по-другому,
// а полмегапикселя — это ещё читаемые 700×700.
const SIZE_STEPS = [
  { pixels: MAX_PIXELS, maxTokens: 1500 },
  { pixels: 900000, maxTokens: 1200 },
  { pixels: 500000, maxTokens: 900 },
];
...
async function fromImage(buffer, mediaType = 'image/jpeg', caption = '', { ad = false } = {}) {
  ...
  // У рекламы к промпту добавляется приписка AD_SUFFIX — ещё около трёхсот
  // токенов, и с обычным потолком сумма вылезает за 8000. Отнимаем их сразу, а
  // не после первого отказа: ступени и так есть, но каждая лишняя попытка —
  // это ещё один поход к модели из тех двух-трёх десятков, что дают сутки.
  const ceiling = (step) => (ad ? Math.min(step.maxTokens, 1200) : step.maxTokens);
...
// Потолок ответа для текста. Groq считает его в минутный лимит целиком, ещё до
// того как модель ответила, — то есть на пересланном объявлении 2600 отнимали
// треть лимита ни за что: столько на один текст не возвращается никогда.
// Ступени на случай, если и этого много: 1400 — это примерно четыре разобранных
// объявления, 800 — одно-два, и до этого доходит только очень длинный текст.
const TEXT_STEPS = [1400, 800];
...
async function fromText(text, { ad = false } = {}) {
  ...
  for (const maxTokens of TEXT_STEPS) {
    try {
      return await ask(task, ad ? AD_SUFFIX : undefined, { maxTokens, image: false });
    } catch (err) {
      // Ужимать нечего — картинки тут нет; всё, чем можно подвинуться, это
      // место, зарезервированное под ответ. Остальные отказы на второй попытке
      // повторятся один в один.
      if (!/слишком большой/i.test(err.message)) throw err;
      lastErr = err;
