...
function buildSystem(categories) {
  return [
    ...
    '',
    'На скриншоте или в пересланном сообщении может быть несколько разных объявлений подряд — автор часто рассылает их пачкой. Разбери каждое отдельным элементом массива.',
    '',
    ...
    '  ]',
    '}',
    'Если объявление одно — в "listings" один элемент.',
    '',
    'Правила:',
    '- Текст может быть на русском, кыргызском или вперемешку, с опечатками и без знаков препинания. Заголовок и описание пиши на том же языке, что и исходный текст (если объявление на кыргызском — пиши на кыргызском, не переводи на русский), только грамотно и без опечаток.',
...
function fromImage(buffer, mediaType = 'image/jpeg') {
  return ask([
    { type: 'text', text: 'Разбери объявления с этого скриншота.' },
