...
function buildSystem(categories) {
  return [
    ...
    '- is_listing = false для работы за границей: заказ или вакансия, где работать не в Кыргызстане (Россия, Казахстан, Корея, Турция, ОАЭ, Европа, США), и набор людей на выезд — «работа в Корее», «вахта в Якутске», «чет өлкөдө жумуш», «визовая поддержка». note «работа за границей». Товар оттуда («одежда из Турции», «машина из Кореи»), туры и грузоперевозки — обычные объявления.',
    ...
    '',
    'Перед ответом по каждому элементу с is_listing = true проверь: понятно, что предлагают или ищут? если заказ или вакансия — названа профессия, а не только график и возраст? если зовут девушек на уборку — назван объект? нет ли здесь запрещённого? работать предлагают в Кыргызстане? Хоть один ответ «нет» — ставь is_listing = false.',
  ].join('\n');
}
...
function normalize(raw) {
  ...
  // Работу за границей не публикуем, даже когда модель её пропустила (см.
  // abroad.js). Заказу и вакансии хватает названной страны или города: работать
  // там, где назвали. Записке на доске нужны ещё и слова про работу — «одежда
  // из Турции» и «туры в Дубай» остаются обычной продажей.
  const job = listing.listing_type === 'order' || listing.listing_type === 'vacancy';
  const found = listing.is_listing
    ? abroadWork([listing.title, listing.description, listing.city].join('\n'), { job })
    : '';
  if (found) {
    listing.is_listing = false;
    listing.note = `работа за границей («${found}»)`;
  }
...
const AD_SUFFIX = [
  ...
  'Исключение одно: запрещённое (наркотики, мошенничество, ставки, займы, документы, оружие, интим-услуги, вербовка девушек под видом уборки или помощи по дому, работа за границей) остаётся запрещённым — для него is_listing = false, как и раньше. За такое объявление могли заплатить точно так же, и это ничего не меняет.',
