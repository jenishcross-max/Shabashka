...
function buildSystem(categories) {
  return [
    ...
    '',
    'ВАЖНО: всё, что ты вернёшь с is_listing = true, публикуется на сайте автоматически, без проверки человеком. Человек увидит объявление уже опубликованным. Поэтому по каждому сообщению строго отвечай на вопрос: «автору нужен человек на работу — разовую (заказ) или постоянную (вакансия)?» Если да — разбирай. Если нет или непонятно — is_listing = false. Пропустить сомнительное правильнее, чем опубликовать лишнее.',
    ...
    '- is_listing = true только если выполнено И то, И другое: в сообщении названа КОНКРЕТНАЯ работа, и автор — тот, кому её надо сделать (заказчик или работодатель), а не тот, кто готов её выполнить. Проверь оба условия отдельно.',
    '- is_listing = false для переписки, приветствий («ассалам алейкум», «salam», «+», «кто свободен?»), благодарностей, споров, пересылок новостей, рекламы товаров, продажи вещей, поиска жилья, услуг такси, обмена валют, займов и сбора денег.',
    '- ОСОБО ВАЖНО: is_listing = false, если человек предлагает СВОИ услуги или ищет работу для себя. «Сантехник, все виды работ, звоните», «Ищу работу сварщиком», «Бригада делает ремонт под ключ», «Опыт 10 лет, недорого» — это исполнители, а не заказчики. Их на сайт публиковать нельзя. Признаки: перечисление своих навыков, «все виды работ», «недорого», «качественно», «опыт», «обращайтесь», отсутствие описания конкретной задачи. Такие сообщения в чатах очень частые — не путай их с заказами.',
    ...
    '- listing_type = "order", если человек ищет исполнителя на разовую работу (в том числе стройка, шабашка); "vacancy", только если явно и однозначно видно, что это постоянная/сменная вакансия с работодателем, графиком или окладом. Если не уверен — ставь "order": большинство объявлений в этих чатах — разовые заказы на стройку. "other" — только для явного мусора (переписка, реклама товаров, поиск жилья).',
    ...
    '- budget: число в сомах — для заказа цена, для вакансии нижняя граница зарплаты. «2000с», «2 000 сом», «2к» → 2000. Если указана вилка — бери нижнюю границу. «Договорная» → пусто.',
    ...
    '',
    'Перед ответом перечитай каждый элемент с is_listing = true и проверь: это заказчик, а не исполнитель? работа названа конкретно? текст ты действительно прочитал, а не додумал? Если хоть на один вопрос ответ «нет» — поставь is_listing = false.',
  ].join('\n');
}
...
function normalize(raw) {
  ...
  return {
    ...
    listing_type: clean(raw.listing_type) || 'order',
