...
function start() {
  ...
  const timer = setInterval(() => {
    ...
    // Срок короткий: это пинг самому себе, и висящее соединение здесь только
    // копило бы сокеты до следующей попытки.
    fetch(`${base}${HEALTH_PATH}`, { signal: AbortSignal.timeout(15000) }).catch(() => {});
  }, INTERVAL_MS);
