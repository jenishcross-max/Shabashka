...
// а Meta может забраковать или не успеть обработать само видео. Объявление из-за
// этого не должно пропасть из ленты совсем: карточка рисуется тем же макетом
// (см. renderStill в card.js), подпись остаётся той же, а ни кодирования, ни
// обработки на стороне Instagram картинке не нужно — то есть отпадает ровно то,
// на чём ролик и споткнулся.
async function withImageFallback(job, failure) {
  ...
  const result = await post('insta', () => instagram.publishImage(url, job.caption));
